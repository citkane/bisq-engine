...
class TakeOfferDataModel extends ActivatableDataModel {
    ...
    protected void activate() {
        ...
        tradeManager.checkOfferAvailability(
            offer,
            () -> {},
            errorMessage -> {
                if(Args.gui) new Popup<>().warning(errorMessage).show();
    ...
    void onTakeOffer(TradeResultHandler tradeResultHandler) {
        ...
        if (filterManager.isCurrencyBanned(offer.getCurrencyCode())) {
            if(Args.gui) new Popup<>().warning(Res.get("offerbook.warning.currencyBanned")).show();
            message.success = false;
            ...
            if(Args.gui) new Popup<>().warning(Res.get("offerbook.warning.paymentMethodBanned")).show();
            message.success = false;
            message.message = Res.get("offerbook.warning.paymentMethodBanned");
        } else if (filterManager.isOfferIdBanned(offer.getId())) {
            if(Args.gui) new Popup<>().warning(Res.get("offerbook.warning.offerBlocked")).show();
            message.success = false;
            ...
            if(Args.gui) new Popup<>().warning(Res.get("offerbook.warning.nodeBlocked")).show();
            message.success = false;
