...
public class EngineBoot {
    ...
    private void startBasicServices() {
        ...
        allServicesDone.subscribe((observable, oldValue, newValue) -> {
            if (newValue) {
                ...
                if (Args.gui && torNetworkSettingsWindow != null)
    ...
    private BooleanProperty initP2PNetwork() {
        ...
        p2PService.start(new P2PServiceListener() {
            ...
            public void onTorNodeReady() {
                ...
                if(Args.gui) p2PNetworkIconId.set("image-connection-tor");
            ...
            public void onDataReceived() {
                ...
                if(Args.gui) splashP2PNetworkAnimationVisible.set(false);
    ...
    private void initWalletService() {
        ...
        walletsSetup.initialize(null,
                () -> {
                    ...
                    if (walletsManager.areWalletsEncrypted()) {
                        if (p2pNetWorkReady.get() && Args.gui)
                            splashP2PNetworkAnimationVisible.set(false);

                        if(Args.gui) walletPasswordWindow
                                .onAesKey(aesKey -> {
    ...
    private void onBasicServicesInitialized() {
        ...
        tradeManager.setTakeOfferRequestErrorMessageHandler(errorMessage -> {
        ...
        if(Args.gui) GUIUtil.setFeeService(feeService);
        ...
        user.getPaymentAccountsAsObservable().addListener((SetChangeListener<PaymentAccount>) change -> {
            if (!walletsManager.areWalletsEncrypted() && preferences.showAgain(key) && change.wasAdded()) {
                if(Args.gui) new Popup<>().headLine(Res.get("popup.securityRecommendation.headline"))
    ...
    private void showSecondPopupIfResyncSPVRequested(Popup firstPopup) {
        if(Args.gui) firstPopup.hide();
        preferences.setResyncSpvRequested(false);
        if(Args.gui) new Popup<>().information(Res.get("settings.net.reSyncSPVAfterRestartCompleted"))
                .hideCloseButton()
                .useShutDownButton()
                .show();
        ...
        Thread checkIfLocalHostNodeIsRunningThread = new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    UserThread.execute(EngineBoot.this::readMapsFromResources);
                } finally {
                    if (socket != null) {
                        try {
                            socket.close();
        ...
        Thread checkCryptoThread = new Thread() {
            ...
            public void run() {
                try {
                    ...
                    log.error(msg);
                    if(Args.gui) UserThread.execute(() -> new Popup<>().warning(msg)
                            .useShutDownButton()
                            .useReportBugButton()
                            .show());
    ...
    public void onSplashScreenRemoved() {
                    ...
                    switch (trade.getTradePeriodState()) {
                        ...
                        case SECOND_HALF:
                            ...
                            if (DontShowAgainLookup.showAgain(key)) {
                                ...
                                if(Args.gui) new Popup<>().warning(Res.get("popup.warning.tradePeriod.halfReached",
                                        trade.getShortId(),
                        ...
                        case TRADE_PERIOD_OVER:
                            ...
                            if (DontShowAgainLookup.showAgain(key)) {
                                DontShowAgainLookup.dontShowAgain(key, true);
                                if(Args.gui) new Popup<>().warning(Res.get("popup.warning.tradePeriod.ended",
                                        trade.getShortId(),
                                        formatter.formatDateTime(maxTradePeriodDate)))
                                        .show();
        ...
        if (alert != null) {
            if (alert.isUpdateInfo()) {
                ...
                if (isNewVersion && (preferences.showAgain(key) || openNewVersionPopup)) {
                    if(Args.gui) new DisplayUpdateDownloadWindow(alert)
                ...
                if (displayedAlert == null || !displayedAlert.equals(alert))
                    if(Args.gui) new DisplayAlertMessageWindow()
                            .alertMessage(alert)
                            .onClose(() -> {
                                user.setDisplayedAlert(alert);
                            })
    ...
    private void displayPrivateNotification(PrivateNotificationPayload privateNotification) {
        if(Args.gui) new Popup<>().headLine(Res.get("popup.privateNotification.headline"))
    ...
    private void removeOffersWithoutAccountAgeWitness() {
        if (new Date().after(AccountAgeWitnessService.FULL_ACTIVATION)) {
            openOfferManager.getObservableList().stream()
                    ...
                    .forEach(e -> {
                        if(Args.gui) new Popup<>().warning(Res.get("popup.warning.offerWithoutAccountAgeWitness", e.getId()))
